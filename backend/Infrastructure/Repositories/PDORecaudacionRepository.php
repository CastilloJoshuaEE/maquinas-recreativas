...
class PDORecaudacionRepository implements RecaudacionRepository
{
    ...
    public function findById(Uuid $id): ?Recaudacion
    {
        ...
        return $this->cache->remember($cacheKey, function () use ($id) {
            $stmt = $this->db->prepareCall('CALL sp_buscar_recaudacion_por_id(?)');
    ...
    public function findInformeByRecaudacion(Uuid $idRecaudacion): ?InformeRecaudacion
    {
        ...
        return $this->cache->remember($cacheKey, function () use ($idRecaudacion) {
            $stmt = $this->db->prepareCall('CALL sp_obtener_informe_por_recaudacion(?)');
    ...
    public function findDetallesByInforme(Uuid $idInforme): array
    {
        ...
        return $this->cache->remember($cacheKey, function () use ($idInforme) {
            $stmt = $this->db->prepareCall('CALL sp_detalles_informe(?)');
    ...
    public function findMaquinasRecaudacion(): array
    {
        ...
        return $this->cache->remember($cacheKey, function () {
            $stmt = $this->db->prepareCall('CALL sp_maquinas_operativas_recaudacion()');
            $stmt->execute();
    ...
    public function save(Recaudacion $recaudacion): void
    {
        ...
        if ($existing) {
            $stmt = $this->db->prepareCall('CALL sp_actualizar_recaudacion(?, ?, ?, ?, ?, ?, ?, ?, ?)');
            $stmt->execute([
                $data['ID_Recaudacion'],
                $data['Tipo_Comercio'],
                $data['ID_Maquina'],
            ...
            $stmt = $this->db->prepareCall('CALL sp_insertar_recaudacion(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
            $stmt->execute([
                $data['ID_Recaudacion'],
                $data['Tipo_Comercio'],
                $data['ID_Maquina'],
    ...
    public function saveInforme(InformeRecaudacion $informe): void
    {
        ...
        $stmt = $this->db->prepareCall('CALL sp_guardar_informe_recaudacion(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([
            $data['ID_Informe'],
            $data['ID_Recaudacion'],
            $data['CI_Usuario'],
    ...
    public function saveDetalle(DetalleInforme $detalle): void
    {
        ...
        $stmt = $this->db->prepareCall('CALL sp_guardar_detalle_informe(?, ?, ?)');
    ...
    {
        $stmt = $this->db->prepareCall('CALL sp_eliminar_recaudacion(?)');
    ...
    public function findAll(array $filters = [], int $limit = 100, int $offset = 0): array
    {
        ...
        return $this->cache->remember($cacheKey, function () use ($filters, $limit, $offset) {
            ...
            $stmt = $this->db->prepareCall('CALL sp_listar_recaudaciones(?, ?, ?, ?, ?, ?)');
    ...
    public function findResumenByTipoComercio(?int $limit = null): array
    {
        ...
        return $this->cache->remember($cacheKey, function () use ($limit) {
            $stmt = $this->db->prepareCall('CALL sp_resumen_recaudaciones(?)');
            $stmt->execute([$limit]);
