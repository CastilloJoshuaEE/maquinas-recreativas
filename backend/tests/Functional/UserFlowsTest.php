...
class UserFlowsTest extends HttpTestCase {
    ...
    public function testFlujoCompletoUsuario() {
        ...
        echo "================================================================\n\n";
        echo "Paso 0: Login como administrador del sistema...\n";
    ...
    private function pasoRegistrarUsuarios() {
        echo "Paso 1: Registrando usuarios (usando administrador)...\n";
        // Registrar logistica
        $this->registrarUsuarioAdmin($this->logisticaUser);
        sleep(1);
        // Registrar ensamblador
        $this->registrarUsuarioAdmin($this->ensambladorUser);
        sleep(1);
        // Registrar comprobador
        $this->registrarUsuarioAdmin($this->comprobadorUser);
        
        // Recuperar los usuarios creados (buscar por email)
        $userLog = $this->buscarUsuarioPorEmail($this->logisticaUser['email']);
        if ($userLog) {
            $this->logisticaId = $userLog['id'];
            $this->logisticaUsuarioAsignado = $userLog['usuario_asignado'];
            echo "   Logistica registrado: {$this->logisticaUsuarioAsignado} (ID: {$this->logisticaId})\n";
        }
        $userEns = $this->buscarUsuarioPorEmail($this->ensambladorUser['email']);
        if ($userEns) {
            $this->ensambladorId = $userEns['id'];
            $this->ensambladorUsuarioAsignado = $userEns['usuario_asignado'];
            echo "   Ensamblador registrado: {$this->ensambladorUsuarioAsignado} (ID: {$this->ensambladorId})\n";
        }
        $userComp = $this->buscarUsuarioPorEmail($this->comprobadorUser['email']);
        if ($userComp) {
            $this->comprobadorId = $userComp['id'];
            $this->comprobadorUsuarioAsignado = $userComp['usuario_asignado'];
    ...
    private function pasoLoginLogistica() {
        ...
        $this->assertResponseSuccess('Error al iniciar sesion como logistica');
    ...
    private function pasoLoginEnsamblador() {
        ...
        $this->assertResponseSuccess('Error al iniciar sesion como ensamblador');
        if ($this->isSuccessResponse($response)) echo "   Login exitoso\n";
    }
    ...
    private function pasoLoginComprobador() {
        ...
        $this->assertResponseSuccess('Error al iniciar sesion como comprobador');
        if ($this->isSuccessResponse($response)) echo "   Login exitoso\n";
    }
    ...
    private function pasoVerMaquinasEnsamblador() {
        ...
        if ($this->assertResponseSuccess('Error al obtener maquinas del ensamblador')) {
            // La respuesta tiene 'maquinas' y cada máquina tiene 'ID_Maquina'
            ...
            foreach ($maquinas as $maquina) {
                // Cambiar 'id' por 'ID_Maquina'
                if (isset($maquina['ID_Maquina']) && $maquina['ID_Maquina'] === $this->maquinaId) {
